...
createIframe = function(url){
	return "<iframe src='"+url+"' width='99%' height='99%'></iframe>";
}
...
/*
 * <!-- 窗口 -->
	<div class="easyui-dialog" id="" data-options="closed:true,modal:true,buttons:[{
		text:'窗口',
		handler:function(){
...
get = function(id,pid,cid){
	$("#"+id).combobox({
		url:'book/queryAddr.do?pid='+pid,
		valueField:'id',
		textField:'name',
		editable:false,
		onChange:function(newValue,oldValue){
			$("#"+cid).combobox({
				url:'book/queryAddr.do?pid='+newValue,
				valueField:'id',
				textField:'name',
				editable:false
			});
...
queryById = function(id,url,formId){
	...
	$.ajax({
		...
		success:function(data){
			...
			$("#imgDom2").prop('src','../upload/'+data.url);
		}
...
//批量删除
delMany = function(tableId,url,flag){
...
//单删
delOne = function(id,flag){
	//queryNameById
	$.ajax({
		url:'book/queryById.do?id='+id,
		success:function(data){
			if (flag) {
				$.messager.confirm('提示','确定删除'+data.name+'吗？',function(boo){
					if (boo) {
						del(id,'book/del.do');
						return
					}
